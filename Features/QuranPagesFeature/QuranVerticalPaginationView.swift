...
struct QuranVerticalPaginationView<Content: View>: View {
    ...
    // Buffer size in *pages*. 20 pages is roughly 300 verses.
    ...
    private var filteredPages: [Page] {
        ...
        // Ensure range is valid for array
    ...
    private func initializeRange() {
        guard let first = selection.first else {
            // Default to first few pages if no selection
        ...
        // Use pageNumber to find index (1-based pageNumber -> 0-based index)
        // This avoids potential `Quran` instance mismatches in Page equality
    ...
    private func handlePageAppear(_ page: Page) {
        // Note: We no longer update selection here to avoid pre-emptive updates.
        // Selection is updated via GeometryReader in onPreferenceChange.
        
        // Dynamic loading logic - ONLY expand downwards
        // Upward expansion causes scroll position jumps in SwiftUI ScrollView
        let index = page.pageNumber - 1
        guard index >= 0, let currentRange = loadedRange else { return }
        
        // Expand downwards only
    ...
    private func handleVisibilityChange(_ visibilities: [PageVisibility], proxy: ScrollViewProxy) {
        // 1. Handle Initial Scroll if Pending - retry scroll until content loads
        if let targetPage = pendingInitialScrollPage {
            // Retry scroll (content may have resized)
            proxy.scrollTo(targetPage, anchor: .top)
            
            // Check if target is visible and has loaded content
            if let visiblePage = visibilities.first(where: { $0.page.pageNumber == targetPage.pageNumber }) {
                if visiblePage.height > 100 {
                    // Content loaded. Clear pending after one more dispatch to ensure final position.
                    DispatchQueue.main.async {
                        self.pendingInitialScrollPage = nil
                    }
                }
            }
            // Still pending - will retry on next visibility change
            return
        }
        
        // 2. Update Selection logic
        // BLOCK updates if we are in default range mode (waiting for correct data)
        guard !isProgrammaticScroll, !isDefaultRange, !visibilities.isEmpty else { return }
        
        // Sort by minY
        let sorted = visibilities.sorted { $0.minY < $1.minY }
        
        // Pick the first page that is effectively "at the top"
        // We look for the first page that starts below -200 (mostly invisible top)
        // or effectively the one crossing the top edge.
    ...
    private func handleSelectionChange(_ newSelection: [Page], proxy: ScrollViewProxy) {
        ...
        // Check if range is nil OR index is outside OR we are in default mode
        let needsRangeUpdate: Bool = {
            guard let range = loadedRange else { return true }
            return isDefaultRange || !range.contains(index)
        }()
        
        // If selection is outside loaded range (or range nil), reset/init range
        if needsRangeUpdate {
             isDefaultRange = false // We have a valid target now
