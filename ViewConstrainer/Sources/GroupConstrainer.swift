...
public class GroupConstrainer: ViewConstrainer {
    ...
    convenience init(constrainer: ViewConstrainer, group: [SingleConstrainer]) {
        let chain = constrainer.chain + group.flatMap { $0.chain }
        self.init(view: constrainer.view, constraints: group.map { $0.constraint }, chain: chain)
    }

    convenience init(constrainer: ViewConstrainer, group: [GroupConstrainer]) {
        let chain = constrainer.chain + group.flatMap { $0.chain }
        self.init(view: constrainer.view, constraints: group.flatMap { $0.constraints }, chain: chain)
    }
}
