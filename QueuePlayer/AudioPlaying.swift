...
struct AudioPlaying {
    ...
    var file: AudioFile {
        return request.files[filePlaying.fileIndex]
    }
    var frame: AudioFrame {
        return file.frames[framePlaying.frameIndex]
    }

    func isLastPlayForCurrentFrame() -> Bool {
        return framePlaying.framePlays + 1 >= request.frameRuns.maxRuns
    }

    func isLastRun() -> Bool {
        return requestPlays + 1 >= request.requestRuns.maxRuns
    }
