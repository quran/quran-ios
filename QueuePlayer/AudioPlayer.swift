...
class AudioPlayer: AudioInterruptionMonitorDelegate, PlayerDelegate {
    ...
    private var timer: VFoundation.Timer? {
    ...
    private func waitUntilFrameEnds(currentTime: TimeInterval? = nil) {
        ...
        let interval = max(0.1, self.getDurationToFrameEnd(currentTime: currentTime))
        self.timer = VFoundation.Timer(interval: interval, queue: .main) { [weak self] in
            self?.timer = nil
            self?.onFrameEnded()
        }
    }
    ...
    private func isPaused() -> Bool {
        return isPaused(rate: player.rate)
    }
