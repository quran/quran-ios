//
//  SynchronizedPageBookmarkPersistenceTests.swift
//  QuranEngine
//
...
final class SynchronizablePageBookmarkPersistenceTests: XCTestCase {
    ...
    override func setUp() {
        ...
        persistence = SynchronizablePageBookmarkPersistence(
            syncedBookmarksPersistence: syncedPersistence,
    ...
    func testMergingSyncedAndMutatedBookmarks() async throws {
        ...
        let expectation1 = expectation(description: "Merging the initial uncolliding bookmarks")
        assertexpectation = expectation1
        expecteBookmarks = [
            .init(remoteID: "remID:1", page: 10, creationDate: .init(timeIntervalSince1970: 1000)),
            .init(remoteID: "remID:2", page: 15, creationDate: .init(timeIntervalSince1970: 2000)),
        ...
        let syncedBookmarks: [SyncedPageBookmarkPersistenceModel] = [
            .init(page: 10, remoteID: "remID:1", creationDate: .init(timeIntervalSince1970: 1000)),
            .init(page: 15, remoteID: "remID:2", creationDate: .init(timeIntervalSince1970: 2000)),
        ]
        ...
        let expectation2 = expectation(description: "Merging: one synced bookmark removed")
        assertexpectation = expectation2
        expecteBookmarks = [
            .init(remoteID: "remID:2", page: 15, creationDate: .init(timeIntervalSince1970: 1000)),
            .init(page: 23, creationDate: Date()),
            .init(page: 25, creationDate: Date()),
            .init(page: 100, creationDate: Date()),
        ]
        ...
        let expectation3 = expectation(description: "Merging: Locally adding the removed bookmark")
    ...
    func testMutationsPublished() async throws {
        ...
        let syncedBookmarks: [SyncedPageBookmarkPersistenceModel] = [
            .init(page: 10, remoteID: "remID:1", creationDate: .init(timeIntervalSince1970: 1000)),
            .init(page: 22, remoteID: "remID:2", creationDate: .init(timeIntervalSince1970: 3000)),
            .init(page: 33, remoteID: "remID:3", creationDate: .init(timeIntervalSince1970: 10000)),
        ]
        for bookmark in syncedBookmarks {
            try await syncedPersistence.insertBookmark(bookmark)
        }
        ...
        // Insert a new one
        let expectation1 = expectation(description: "After creating a new bookmark")
        assertexpectation = expectation1
        ...
        let expectation2 = expectation(description: "After creating some and removing some")
        assertexpectation = expectation2
    ...
    func testDeletionsSideEffects() async throws {
        ...
        try await AsyncAssertEqual(
            try await localMutationsPersistence.bookmarks().first?.page,
            22,
            "Expected to have a record for page 22 in the local mutations persistence"
        )
        try await AsyncAssertEqual(
            try await localMutationsPersistence.bookmarks().first?.remoteID,
            "remID:2",
            "Expected to have a the correct remote ID"
        )
        try await AsyncAssertEqual(
            try await localMutationsPersistence.bookmarks().first?.mutation,
            .deleted,
            "Expected to have the correct mutation event."
        )
        try await AsyncAssertEqual(
            try await syncedPersistence.bookmark(page: 22) != nil,
            true,
    ...
    func testCreationsSideEffects() async throws {
        ...
        try await AsyncAssertEqual(
            try await syncedPersistence.bookmark(page: 123) == nil,
            true,
        ...
        await AsyncAssertThrows(
            try await persistence.insertPageBookmark(10),
            nil,
